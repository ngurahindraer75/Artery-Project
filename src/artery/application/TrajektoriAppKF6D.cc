#include "artery/application/TrajektoriAppKF6D.h"
...
Define_Module(TrajektoriAppKF6D);

TrajektoriAppKF6D::~TrajektoriAppKF6D() {
...
std::string TrajektoriAppKF6D::getNodeType() {
...
void TrajektoriAppKF6D::initialize() {
...
void TrajektoriAppKF6D::handleMessage(cMessage* msg) {
    if (msg == mLogTimer) {
        logTrajectory();
        scheduleAt(simTime() + mLogInterval, mLogTimer);
        return;
...
void TrajektoriAppKF6D::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details) {
...
void TrajektoriAppKF6D::evaluatePendingPredictions(long targetId, AgentHistory& hist_struct) {
...
void TrajektoriAppKF6D::logTrajectory() {
    try {
...
void TrajektoriAppKF6D::takeKfSnapshot() {
...
void TrajektoriAppKF6D::latLonToCartesian(double lat_micro, double lon_micro, double ref_lat_micro, double ref_lon_micro, double& x, double& y) {
...
void TrajektoriAppKF6D::cartesianToLatLon(double x, double y, double ref_lat_micro, double ref_lon_micro, double& lat_micro, double& lon_micro) {
    const double R = 6371000.0;
    double ref_lat = ref_lat_micro / 1000000.0;
    double ref_lon = ref_lon_micro / 1000000.0;
    double ref_lat_rad = ref_lat * M_PI / 180.0;
