...
void TrajAppKF4D::initialize() {
    ...
    auto initCamLog = [&](std::ofstream& stream, const std::string& targetLabel) {
        stream.open("results/V4_KF4D_CAM_Target_" + targetLabel + ".csv", std::ios_base::out | std::ios_base::app);
    ...
    auto initPredLog = [&](std::ofstream& stream, const std::string& targetLabel, const std::string& horizonStr) {
        stream.open("results/V4_KF4D_" + horizonStr + "s_coefficient_log_Target_" + targetLabel + ".csv", std::ios_base::out | std::ios_base::app);
...
void TrajAppKF4D::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details) {
    ...
    if (!agent.is_ref_set) {
        ...
        agent.kf_state = std::make_unique<KalmanFilter4D>(q_tuning_factor);
    ...
    evaluatePendingPredictionsKF4D(target_id, agent);
}
...
void TrajAppKF4D::takeKfSnapshot() {
    ...
    for (auto& pair: mOtherNodes) {
        ...
        // FIX: Defensive programming bound check to prevent OMNeT++ Segmentation Faults
        if (state.size() >= 4) { 
