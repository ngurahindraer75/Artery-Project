...
void TrajektoriApp::initialize() {
    ...
    std::string logFilename = (nodeType == "Vehicle") ? "results/CAM_data_from_car.csv" : "results/CAM_data_from_person.csv";
    ...
    std::string coefficientLogFilename = (nodeType == "Vehicle") ? "results/coefficient_log_from_car.csv" : "results/coefficient_log_from_person.csv";
    mCoefficientLogFile.open(coefficientLogFilename, std::ios::out | std::ios::app);
    if (mCoefficientLogFile.tellp() == 0) {
        mCoefficientLogFile << "Processing_Time(s);Latest_CAM_Time(s);Target_Prediction_Time(s);Node_Target;Actual_Lat;Actual_Lon;Slope_Lat;Intercept_Lat;Pred_Lat;Slope_Lon;Intercept_Lon;Pred_Lon;Lat_AE;Lon_AE;AE" << std::endl;
    }
...
void TrajektoriApp::runLinearRegression() {
    ...
    for (auto& pair : mOtherNodes) {
        ...
        MovementData current_latest_data = hist.back();
        double current_latest_cam_time = current_latest_data.timestamp.dbl(); 

        // ===================================================================================
        // USER LOGIC STEP 2, 3, 5, 6: VALIDATE PREVIOUS CYCLE PENDING PREDICTION
        // ===================================================================================
        if (hist_struct.pending.is_active) {
            ...
            if (target_prediction_time - hist_struct.pending.latest_cam_time >= 0.5) {
                ...
                // Step 3: Run Linear Regression using the 1-second window collected from the PREVIOUS cycle
                auto& train_data = hist_struct.pending.train_data;
                ...
                if (train_data.size() >= 2) {
                    ...
                    for (auto& pt : train_data) {
                        ...
                        sum_x += x; sum_y_lat += y_lat; sum_y_lon += y_lon;
                        sum_xx += x * x; sum_xy_lat += x * y_lat; sum_xy_lon += x * y_lon;
                    }
                    ...
                    if (std::abs(denominator) > 1e-9) {
                        ...
                        // Project to the Target Time found at current cycle
                        ...
                        // Print to CSV. Crucially, Processing_Time and Latest_CAM_Time use the PENDING values!
                        if (mCoefficientLogFile.is_open()) {
                            mCoefficientLogFile << std::fixed << std::setprecision(12)
                                                ...
                                                << b_lat << ";" << a_lat << ";" << pred_lat << ";"                  
                                                << b_lon << ";" << a_lon << ";" << pred_lon << ";"
        ...
        // ===================================================================================
        // USER LOGIC STEP 1 & 3: RECORD PROCESSING TIME, BASE TIME, AND TRAINING WINDOW
        // ===================================================================================
        hist_struct.pending.processing_time = t_sim; // E.g., 1.1
        hist_struct.pending.latest_cam_time = current_latest_cam_time; // E.g., 0.838

        // Define 1-Second Training Window just behind Latest_CAM_Time (Between 0 and 1 relative)
        double t_start = current_latest_cam_time - 1.0; 
        double t_end = current_latest_cam_time;
        ...
        if (train_data.size() < 2 && has_fallback) train_data.insert(train_data.begin(), fallback_point);
        
        // Store it inside the Pending Buffer, waiting for the next timer cycle to project it
        hist_struct.pending.train_data = train_data;
...
void TrajektoriApp::finish() {
    ...
    if (mCountAE > 0 && mCoefficientLogFile.is_open()) {
