#include "artery/application/TrajektoriAppKFv5.h"
...
Define_Module(TrajektoriAppKFv5);

TrajektoriAppKFv5::~TrajektoriAppKFv5() {
...
std::string TrajektoriAppKFv5::getNodeType() {
...
void TrajektoriAppKFv5::initialize() {
...
void TrajektoriAppKFv5::handleMessage(cMessage* msg) {
    if (msg == mLogTimer) {
        logTrajectory();
        scheduleAt(simTime() + mLogInterval, mLogTimer);
        return;
...
void TrajektoriAppKFv5::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details) {
...
void TrajektoriAppKFv5::evaluatePendingPredictions(long targetId, AgentHistory& hist_struct) {
...
void TrajektoriAppKFv5::logTrajectory() {
    try {
...
void TrajektoriAppKFv5::takeKfSnapshot() {
...
void TrajektoriAppKFv5::latLonToCartesian(double lat_micro, double lon_micro, double ref_lat_micro, double ref_lon_micro, double& x, double& y) {
...
void TrajektoriAppKFv5::cartesianToLatLon(double x, double y, double ref_lat_micro, double ref_lon_micro, double& lat_micro, double& lon_micro) {
    const double R = 6371000.0;
    double ref_lat = ref_lat_micro / 1000000.0;
    double ref_lon = ref_lon_micro / 1000000.0;
