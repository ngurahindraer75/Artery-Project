...
// Memprediksi trajektori dengan mengisi data yang hilang (padding)
void TrajektoriApp::predictTrajectory()
{
    ...
    for (auto& [targetId, hist] : mOtherNodes) {
        // Lewati jika tidak ada riwayat sama sekali untuk target ini
        if (hist.history.empty()) {
            continue;
        }

        // =========================================================================
        // LOGIKA BARU: PADDING DATA UNTUK MENDAPATKAN 11 TITIK
        // =========================================================================
        const int TARGET_POINTS = 11;
        const double TIME_STEP = 0.1;
        std::deque<MovementData> prediction_points;

        // 1. Dapatkan data terakhir yang valid untuk digunakan sebagai pengisi (padding)
        MovementData padding_point = hist.history.back();

        // 2. Petakan titik yang diterima ke slot waktu 0.1s untuk menghindari duplikat
        std::map<int, MovementData> point_map;
        double window_start_time = simTime().dbl() - 1.0;

        for (const auto& d : hist.history) {
            if (d.timestamp.dbl() >= window_start_time) {
                // Tentukan slot waktu (0-10) untuk setiap titik data
                int slot = static_cast<int>(round((d.timestamp.dbl() - window_start_time) / TIME_STEP));
                if (slot >= 0 && slot < TARGET_POINTS) {
                    point_map[slot] = d;
                }
            }
        }

        // 3. Bangun 11 titik data, isi kekosongan jika ada
        for (int i = 0; i < TARGET_POINTS; ++i) {
            auto it = point_map.find(i);
            if (it != point_map.end()) {
                // Jika ada data nyata untuk slot ini, gunakan
                prediction_points.push_back(it->second);
            } else {
                // Jika tidak ada data, buat titik palsu (padding)
                MovementData filled_point = padding_point;
                // Atur timestamp-nya agar sesuai dengan slot waktu yang diharapkan
                filled_point.timestamp = window_start_time + (i * TIME_STEP);
                prediction_points.push_back(filled_point);
            }
        }

        // =========================================================================
        // Lakukan prediksi jika kita berhasil membuat 11 titik data
        // =========================================================================
        if (prediction_points.size() == TARGET_POINTS) {
